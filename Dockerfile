...
RUN \
       apt-get -qq update && \
       apt-get install -y ack awscli curl dnsutils git htop inetutils-ping mtr nano netcat nmap  openjdk-8-jre-headless python python3 python3-pip ruby-full sed tcpdump util-linux vim wget && rm -rf /var/lib/apt/lists/* && apt-get clean && \
       mkdir -p /tools/kafka && \
       cd /tools/kafka/ && \
